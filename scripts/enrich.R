...
DEG<-read.delim("gene_count_matrix.csv.R25C_vs_R32C.edgeR.DE_results",header = TRUE)
DEG$sampleA<-gsub("R25C", "25°C", DEG$sampleA)
DEG$sampleB<-gsub("R32C", "32°C", DEG$sampleB)
DEG$Groups<-paste0(DEG$sampleA, ">", DEG$sampleB)
DEG$change<-NA
DEG$change[DEG$FDR>=0.05]="ns"
DEG$change[DEG$logFC<0 & DEG$FDR<0.05]<-"Up"
DEG$change[DEG$logFC>0 & DEG$FDR<0.05]<-"Down"
...
GenesKEGGpair.1v1<-read.delim("D:/oyster/eggnog/KEGG.1v1.txt", header = TRUE)
GenesGOpair.1v1<-read.delim("D:/oyster/eggnog/GO.1v1.txt", header = TRUE)
Geneskopair.1v1<-read.delim("D:/oyster/eggnog/KO.1v1.txt", header = TRUE)
...
####KOG enrich
{
  ...
  list(plotdata$Group)
  plotdata$ratio2[plotdata$change=="Down"]<- (0-plotdata$ratio2[plotdata$change=="Down"])
  a<-plotdata %>%
  ...
  topptx(a,f, width = 10, height = , units = "in")
}
...
####GO enrich
{
  ...
  plotdata1<-subset(plotdata1, abs(ratio2)>0 & p.adjust< 0.1 )
  plotdata1$ratio2[plotdata1$change=="Down"]<- (0-plotdata1$ratio2[plotdata1$change=="Down"])
  ...
  ggsave(paste0(ref,"GO.tiff"), width = 12, height = 16, units = "in", dpi = 300)
  ggsave(paste0(ref,"GO.png"), width = 12, height = 16, units = "in", dpi = 300)
...
library(edgeR)
rawcount<-read.delim("transcript_count_matrix.txt", header = TRUE)
TMM<-rawcount
data=as.matrix(TMM) 
...
heatmap.input<-TMM.norm.log2[row.names(TMM.norm.log2) %in% unique(DEG$Genes[DEG$change!="ns"]),]
heatmap.input<-heatmap.input[row.names(heatmap.input) %in% unique(hsplist$Genes),]
heatmap.input$Genes<-row.names(heatmap.input)
heatmap.input<-merge(heatmap.input, hsplist, by = "Genes", all.x = T)
row.names(heatmap.input)<-paste0(heatmap.input$Genes, ": ",heatmap.input$Gene.name)
heatmap.input<-heatmap.input[,2:5]

names(heatmap.input)=c("25°C.rep1","25°C.rep2","32°C.rep1","32°C.rep2")
...
heatp<-pheatmap(heatmap.input,
                ...
                cutree_rows = 2, treeheight_row = 0,
                border_color = NA,
                cellwidth = 50, cellheight = 14, fontsize = 10,
                angle_col = c("0"),
                width = 10, height = 6,
                show_colnames = TRUE,
                show_rownames = TRUE,
                filename = "heatmap.hsp.png")
...
DEG.matrix$logFC<-0-DEG.matrix$logFC
...
write.table(DEG.ko.matrix, "DEG.ko.matrix.txt",
            sep = "\t", quote = F, row.names = T)
setwd("pv/")
library(pathview)
...
DEG$Genes<-row.names(DEG)
rawcount<-read.delim("gene_count_log2TMMmatrix.txt", header = T)
rawcount$Genes<-row.names(rawcount)

exp.matrix<-merge(rawcount, DEG, by = "Genes", all = T)
exp.matrix$logFC<- (0-exp.matrix$logFC)
write.csv(exp.matrix, file= "supp.exp.csv", quote = F, row.names = F)
